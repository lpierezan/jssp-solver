...
public:
	static bool check(vi v, int min = 1, int max = -1) {
		...
		for (int i = 0; i < v.size(); i++) {
			int p = v[i] - min; if (p < 0 || p >= mark.size()) return false;
...
public:
	...
	void build_mch_neighborhood(vvi &mch_perm, vector<int> &ms, vector<int> &mp) {
		ms.resize(nop + 1, 0); mp.resize(nop + 1, 0);
	...
	bool compute_fo(vvi mch_perm, int &fo) {
		...
		mark.resize(n + 1); for (int i = 1; i <= n; i++) mark[i].resize(m + 1, 0);
	...
	void build_active_scheduling(vi &op_perm, vvi &mch_perm) {
		...
		mch_perm.clear(); mch_perm.resize(m);
	...
	bool hard_check(vvi &machine_perm, int cmp_fo = I32_INF) {
		if (machine_perm.size() != m) return false;
		for (int mi = 0; mi < m; mi++) {
			if (machine_perm[mi].size() != n) return false;
	...
	bool check_mch_ng(int v, vi &ms, vi &mp) {
		...
		int prox = ms[v]; auto pp = pair_job_mch(prox);
		int ant = mp[v]; auto pa = pair_job_mch(ant);
...
public:
	...
	vi tsv, tsv_inv; vvi my_mch_perm;
	int fo_online = I32_INF;
	bool valid = false;

	solution() { valid = false; my_inst = 0; };
	...
	void associate(instance &inst) {
		my_inst = &inst;
		n = my_inst->n; m = my_inst->m; nop = my_inst->nop;
		valid = false;
	}

	bool config(vvi &mch_perm) {
		if (my_inst == 0) { cout << "No instance associated error."; exit(1); }
		fo_online = I32_INF;
	...
	bool repair() {
		if (valid) return true; //nothing todo

		vi job_perm; job_perm.reserve(nop);
		int mi, pi;
		for (pi = 0; pi < n; pi++) {
			for (mi = 0; mi < m; mi++) {
		...
		if (!ok) { cout << "not expected"; exit(1); }
		return ok;
	}
...
private:
	...
	bool build_ts() {
		...
		if (tsv.size() != nop) return false;

		reverse(ALL(tsv));
		tsv_inv.resize(tsv.size() + 1);
		for (int i = 0; i < tsv.size(); i++) tsv_inv[tsv[i]] = i;
	...
	bool build_dists() {
		...
		for (int i = 0; i<tsv.size(); i++) {
			...
			for (int nv = 1; nv <= 2; nv++) {
				if (nv == 1) pred = pred_pair.first;
				else pred = pred_pair.second;
		...
		for (int i = tsv.size() - 1; i >= 0; i--) {
			...
			for (int nv = 1; nv <= 2; nv++) {
				if (nv == 1) succ = succ_pair.first;
				else succ = succ_pair.second;
	...
	public:
		...
		struct eqp_cmp {
			bool operator() (const move_mem_data &move_mem1, const move_mem_data &move_mem2) const {
				if (move_mem2.fw != move_mem1.fw) return false;
				if (move_mem1.block_idxs.size() != move_mem2.block_idxs.size()) return false;
		...
		string to_string() {
			string ret = (fw ? "fw " : "bw ");
			for (int x : block_idxs) { ret += std::to_string(x) + " "; }
			return ret;
		}
	};
...
public:
	...
	void run_ls(solution &initial_sol) {
		...
		n = initial_sol.n; m = initial_sol.m; nop = initial_sol.nop;
		s = &initial_sol;
...
private:
	...
	void tabu_list_size_update() {
		if (!params.change_tabu_list_size) return;
	...
	void explore_block_moves(vi &block_idxs,
		...
		*/
		...
		for (int i = 1; i<block_size; i++) {
			...
			int j, tmp;
			for (j = i - 1; j >= 1; j--) {
				int a = block_idxs[j]; int asuc = js[a];
			...
			bool is_new_best = (best_eval == -1
				|| (eval < best_eval)
				|| (break_even && (eval == best_eval) && s->is_critical(js[v])));
			...
			if (is_new_best) {
				...
				if (tabu_accepted) {
					...
					if (params.debug_mode) { cout << "New best." << endl; }
				}
				else {
					if (params.debug_mode) {
	...
	bool search_move(move &selected_move) {
		...
		*/
		...
		move best_move; int best_eval = -1;
		...
		if (best_eval != -1) {
			...
			if (params.not_down_moves && best_eval > s->fo_online) {
				found_move = false;
				if (params.debug_mode) {
					cout << "Down Move - Critical Path Move Rejected: " <<
						selected_move.to_string(s->my_inst) << " eval = " << best_eval << endl;
				}
			}
			else {
				found_move = true;
				...
				if (params.debug_mode) {
					cout << "Critical Path Move Selected: " <<
						selected_move.to_string(s->my_inst) << " eval = " << best_eval << endl;
				}
			}
			...
			if (params.debug_mode) { cout << "No move found in critical path." << endl; }
			found_move = false;
		}
	...
	void update_tsv(int u, int v, bool fw,
		...
		//update tsv_inv
		for (i = u_pos; i != (v_pos + inc); i += inc) tsv_inv[tsv[i]] = i;
	...
	void apply_move(move &move_apply) {
		...
		*/
		...
		//=== Update mp,ms
		if (mp[u] != 0) ms[mp[u]] = ms[u];
		if (ms[u] != 0) mp[ms[u]] = mp[u];
		if (fw) {//u in front of v
			mp[u] = v; ms[u] = ms[v];
			ms[v] = u;
			if (ms[u] != 0) mp[ms[u]] = u;
		}
...
public:
	...
	public:
		...
		string to_string(instance &inst) {
			stringstream ret; ret << "Makespan = " << fitness << endl;
	...
	public:
		...
		void clear() {
			for (auto p : indv_pointers) delete p;
		...
		void pop_back() {
			if (indv_pointers.size() == 0) return;
		...
		void sort() {
			...
			std::sort(ALL(indv_pointers),
				...
				bool {return p1->fitness < p2->fitness; });
		...
		const individual* select_bests(int n_elite) {
			if (!sorted) { cout << "Not sorted."; exit(1); }
			n_elite = min(n_elite, (int)indv_pointers.size());
			if (n_elite == 0) return 0;
...
public:
	...
	solution run(instance &inst_) {
		...
		stats.init(inst_);
		//pre-allocation
		int n = inst->n; int m = inst->m;
		aux1.resize(m + 1); for (int i = 0; i <= m; i++) aux1[i].resize(n + 1, 0);
		//run
		run_mh();
		return stats.best_sol_found;
	}
...
private:
	...
	void update_best_fitness(population &pop) {
		...
		if (pop_best < stats.best_fo_found) {
			...
			cout << "Error. Makespan decreased."; exit(1);
		}
	...
	void apply_path_move(int &curr_dist, vvi &curr_indv, const vvi &target_indv, vector<pair<int, int>> &wrong_pos, vvi &f) {
		...
		while (!move_applyed && wrong_pos.size() > 0) {
			...
			f[mi][ji] = pi; f[mi][j2] = p2;

			if (params.log_mode) {
				cout << "After swap move" << endl << inst->mch_perm_to_str(curr_indv)
					<< endl;
			}
	...
	int distance(const individual &p1, const individual &p2) {
		...
		int n = inst->n, m = inst->m; int nop = inst->nop;
...
public:
	//Casa
	string insts_folder_path = "C:\\Users\\Lucas\\Estudos\\PucRio\\Metaheuristicas\\Project 2\\Instances\\";
	//Trab
	//string insts_folder_path = "C:\\Users\\bhq2\\Estudos\\Puc-Rio\\Metaheuristics\\Projeto 2\\Instances\\";
...
void do_test_intances_check() {
	...
	while (true) {
		...
		string solpath = inst_selector.insts_folder_path
			+ inst_selector.signature() + ".sol";
		...
		int fo; inst.compute_fo(mch_perm, fo);
...
void do_inst_ls_solve() {
	...
	while (true) {
		...
		for (int it = 1; it <= n_try; it++) {
			s = sol_gen.generate();
			ls.run_ls(s);
			if (ls.stats.stop_because_no_move) n_stop_no_move++;
...
void do_simple_test_mh() {
	...
	while (true) {
		...
		if (s.fo_online != mh.get_best_fo()) {
			cout << "Error s.fo_online != mh.get_best_fo() "; exit(1);
		}
...
int main()
{
	...
	// =========== IO config =================
	std::ofstream out("C:\\Users\\Lucas\\Google Drive\\jssp_out.txt");
